...
const Sidebar: React.FC<SidebarProps> = React.memo(({ 
  activeSection, onNavigate, lang, onLangToggle, tmcLevel, levelProgress, onOpenLevelInfo 
}) => {
      ...
      <div className="p-6 flex flex-col gap-6 relative flex-1 min-w-[18rem]">
        ...
        <div onClick={onLangToggle} className="relative w-full h-10 bg-slate-900 rounded-xl border border-white/10 cursor-pointer p-1 flex items-center shadow-inner overflow-hidden group/switch hover:border-white/30 transition-colors">
          ...
          <div className="flex w-full justify-between px-3 text-[10px] font-black z-20 pointer-events-none relative select-none">
             <span className={`flex-1 text-center transition-colors duration-300 ${lang === 'es' ? 'text-white' : 'text-slate-600 group-hover/switch:text-slate-400'}`}>ES</span>
             <span className={`flex-1 text-center transition-colors duration-300 ${lang === 'en' ? 'text-white' : 'text-slate-600 group-hover/switch:text-slate-400'}`}>EN</span>
          </div>
        </div>
