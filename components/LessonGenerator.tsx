...
// Simple JSON animation for "Thinking/Loading" (Dot Lottie style)
  ...
  layers: [
    {
      ...
      ks: {
        o: { a: 0, k: 100, ix: 11 },
        r: { a: 0, k: 0, ix: 10 },
        p: { a: 0, k: [20, 50, 0], ix: 2 },
        a: { a: 0, k: [0, 0, 0], ix: 1 },
        s: { 
            a: 1, 
            k: [
                { i: { x: [0.833, 0.833], y: [0.833, 0.833] }, o: { x: [0.167, 0.167], y: [0.167, 0.167] }, t: 0, s: [0, 0, 100] },
                { i: { x: [0.833, 0.833], y: [0.833, 0.833] }, o: { x: [0.167, 0.167], y: [0.167, 0.167] }, t: 30, s: [100, 100, 100] },
                { t: 60, s: [0, 0, 100] }
            ], 
            ix: 6 
        }
      },
      shapes: [{ ty: "el", p: { a: 0, k: [0, 0], ix: 2 }, s: { a: 0, k: [20, 20], ix: 3 }, nm: "Ellipse Path 1" }, { ty: "fl", c: { a: 0, k: [0.23, 0.51, 0.96, 1], ix: 4 }, o: { a: 0, k: 100, ix: 5 }, r: 1, nm: "Fill 1" }]
    },
    {
      ddd: 0,
      ind: 2,
      ...
      ks: {
        ...
        s: { 
            a: 1, 
            k: [
                { i: { x: [0.833, 0.833], y: [0.833, 0.833] }, o: { x: [0.167, 0.167], y: [0.167, 0.167] }, t: 10, s: [0, 0, 100] },
                { i: { x: [0.833, 0.833], y: [0.833, 0.833] }, o: { x: [0.167, 0.167], y: [0.167, 0.167] }, t: 40, s: [100, 100, 100] },
                { t: 70, s: [0, 0, 100] }
            ], 
            ix: 6 
        }
      },
      shapes: [{ ty: "el", p: { a: 0, k: [0, 0], ix: 2 }, s: { a: 0, k: [20, 20], ix: 3 }, nm: "Ellipse Path 1" }, { ty: "fl", c: { a: 0, k: [0.34, 0.8, 0.95, 1], ix: 4 }, o: { a: 0, k: 100, ix: 5 }, r: 1, nm: "Fill 1" }]
    },
    {
      ddd: 0,
      ind: 3,
      ...
      ks: {
        o: { a: 0, k: 100, ix: 11 },
        r: { a: 0, k: 0, ix: 10 },
        p: { a: 0, k: [80, 50, 0], ix: 2 },
        a: { a: 0, k: [0, 0, 0], ix: 1 },
        s: { 
            a: 1, 
            k: [
                { i: { x: [0.833, 0.833], y: [0.833, 0.833] }, o: { x: [0.167, 0.167], y: [0.167, 0.167] }, t: 20, s: [0, 0, 100] },
                { i: { x: [0.833, 0.833], y: [0.833, 0.833] }, o: { x: [0.167, 0.167], y: [0.167, 0.167] }, t: 50, s: [100, 100, 100] },
                { t: 80, s: [0, 0, 100] }
            ], 
            ix: 6 
        }
      },
      shapes: [{ ty: "el", p: { a: 0, k: [0, 0], ix: 2 }, s: { a: 0, k: [20, 20], ix: 3 }, nm: "Ellipse Path 1" }, { ty: "fl", c: { a: 0, k: [0.5, 0.2, 0.9, 1], ix: 4 }, o: { a: 0, k: 100, ix: 5 }, r: 1, nm: "Fill 1" }]
    }
  ]
};
...
const LessonGenerator: React.FC<LessonGeneratorProps> = ({ lang, userTier = 'Novice' }) => {
  ...
  const [personalAnswer, setPersonalCategoryAnswer] = useState('');
  ...
  useEffect(() => {
    ...
    if (history) {
      try { setSavedLessons(JSON.parse(history)); } catch (e) { console.error("Error loading history", e); }
    }
  ...
  const text = {
    ...
    subtitle: lang === 'es' ? 'Camino progresivo desde Principiante hasta Fluidez Profesional.' : 'Progressive path from Beginner to Professional Fluency.',
    quickMission: lang === 'es' ? 'Misión Rápida' : 'Quick Mission',
    deepDive: lang === 'es' ? 'Deep Dive' : 'Deep Dive',
    topicLabel: lang === 'es' ? '¿Qué quieres aprender hoy?' : 'What do you want to learn today?',
    topicPlaceholder: lang === 'es' ? 'Ej: Pedir café, Negocios, Viajes...' : 'Ex: Ordering coffee, Business, Travel...',
  ...
  const handleGenerate = async (levelOverride?: number) => {
    if (!topic.trim()) { setInputError(true); return; }
    setInputError(false);
    setLoading(true);
    setLesson(null);
    setQuizScore(null);
    ...
    try {
      const result = await generateLesson(topic, levelToUse, lang, userTier as 'Novice' | 'Semi Pro' | 'Pro');
      const newSavedLesson: SavedLesson = { ...result, id: Date.now().toString(), dateSaved: Date.now(), numericLevel: levelToUse };
      const updatedHistory = [newSavedLesson, ...savedLessons].slice(0, 20);
  ...
  const checkQuiz = () => {
    ...
    activeQuestionIndices.forEach((questionIndex) => {
      ...
      if (userAnswers[questionIndex] === q.answer) { roundScore++; } else { currentMissed.push(questionIndex); }
    });
  ...
  const submitPersonalAnswer = async () => {
    ...
    } catch (e) { console.error(e); finishPersonalInteraction(); } finally { setIsAiResponding(false); }
  };

  const finishPersonalInteraction = () => {
    setShowPersonalModal(false);
    setAiPersonalResponse(null);
    if (masteryLevel > 1 && masteryLevel % 5 === 0) { setShowUpsellModal(true); } else { handleGenerate(masteryLevel); }
  };

  const handleRetry = () => {
    setActiveQuestionIndices(missedIndices);
    setUserAnswers(prev => { const newAnswers = { ...prev }; missedIndices.forEach(idx => delete newAnswers[idx]); return newAnswers; });
    setQuizScore(null);
    setMissedIndices([]);
  ...
  const triggerFlyingParticles = () => {
    ...
    for (let i = 0; i < 20; i++) {
      particles.push({ id: Date.now() + i, x: window.innerWidth / 2, y: window.innerHeight / 2, targetX: targetRect.left + targetRect.width / 2, targetY: targetRect.top + targetRect.height / 2, icon: '⭐' });
    }
    setFlyingParticles(particles);
    setTimeout(() => { window.dispatchEvent(new Event('tmc-xp-gain')); setFlyingParticles([]); }, 1000); 
  };

  return (
    <div className="space-y-10 pb-24 relative font-sans">
      <div className="fixed inset-0 pointer-events-none z-[100]">
        <AnimatePresence>
          {flyingParticles.map((p, i) => (
            <motion.div key={p.id} initial={{ x: p.x, y: p.y, scale: 0, opacity: 0 }} animate={{ x: p.targetX, y: p.targetY, scale: [1.5, 0.5], opacity: [1, 1, 0] }} transition={{ duration: 0.8, delay: i * 0.05, ease: "easeInOut" }} className="absolute text-4xl">{p.icon}</motion.div>
          ))}
        </AnimatePresence>
      </div>
      ...
      <AnimatePresence>
        {showLevelUp && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
             <motion.div initial={{ scale: 0.5 }} animate={{ scale: 1 }} exit={{ scale: 0.5 }} className="bg-slate-900 p-8 rounded-[48px] shadow-2xl border-4 border-yellow-400 text-center relative overflow-hidden max-w-lg w-full">
                <motion.h2 animate={{ scale: [1, 1.05, 1] }} transition={{ repeat: Infinity, duration: 1.5 }} className="text-4xl font-display font-black text-yellow-400 mb-4">{text.levelUp}</motion.h2>
                <div className="text-6xl mb-6">⭐</div>
                <p className="text-white/90 text-xl font-bold uppercase tracking-widest mb-2">{text.nowLevel}</p>
                <p className="text-6xl font-black text-white mb-8 font-display">{masteryLevel}</p>
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={startPersonalInteraction} className="w-full py-5 bg-blue-600 hover:bg-blue-500 text-white text-xl font-black rounded-3xl shadow-xl">{text.nextLevelBtn} →</motion.button>
             </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {showPersonalModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-slate-900 border border-white/10 rounded-[40px] p-8 max-w-md w-full shadow-2xl">
              {personalQStep === 'ask' ? (
                <>
                   <h3 className="text-2xl font-display font-black text-white mb-2">{text.personalTitle}</h3>
                   ...
                   <input type="text" value={personalAnswer} onChange={e => setPersonalCategoryAnswer(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white mb-6 outline-none focus:border-brand-500" autoFocus />
                   <button onClick={submitPersonalAnswer} disabled={isAiResponding} className="w-full py-4 bg-brand-600 text-white rounded-xl font-black shadow-lg">{isAiResponding ? '...' : text.submit}</button>
                </>
              ) : (
                <>
                   ...
                   <div className="bg-white/5 p-4 rounded-xl border-l-4 border-brand-500 mb-6"><p className="text-slate-300 italic text-sm">{aiPersonalResponse?.fact}</p></div>
                   <button onClick={finishPersonalInteraction} className="w-full py-4 bg-white text-slate-900 rounded-xl font-black shadow-lg">{text.nextLevelBtn}</button>
                </>
              )}
            </motion.div>
          </motion.div>
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <motion.h2 initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="text-4xl md:text-5xl font-display font-black text-white tracking-tighter">{text.title}</motion.h2>
          <p className="text-slate-400 mt-2 text-lg font-sans">{text.subtitle}</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setShowHistory(true)} className="px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest bg-slate-800 text-slate-300 hover:text-white border border-white/5">{text.historyBtn}</button>
          {lesson && (
            <div className="flex bg-slate-900/50 p-1.5 rounded-2xl border border-slate-800 backdrop-blur-md">
              <button onClick={() => setViewMode('quick')} className={`px-6 py-2 rounded-xl text-xs font-black transition-all uppercase tracking-widest ${viewMode === 'quick' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-white'}`}>{text.quickMission}</button>
              <button onClick={() => setViewMode('deep')} className={`px-6 py-2 rounded-xl text-xs font-black transition-all uppercase tracking-widest ${viewMode === 'deep' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-white'}`}>{text.deepDive}</button>
            </div>
          )}
        </div>
      </header>
      ...
      <motion.div layout className="bg-slate-900/40 p-8 md:p-10 rounded-[40px] border border-slate-800 shadow-2xl flex flex-col xl:flex-row gap-8 items-end backdrop-blur-md relative overflow-hidden">
        <div className="flex-1 space-y-3 w-full relative z-10">
          ...
          <input type="text" placeholder={text.topicPlaceholder} className={`w-full px-8 py-5 bg-slate-950 border rounded-2xl text-white outline-none transition-all placeholder:text-slate-600 text-lg shadow-inner ${inputError ? 'border-red-500 animate-pulse' : 'border-slate-700/50 focus:border-blue-500'}`} value={topic} onChange={(e) => { setTopic(e.target.value); if (e.target.value.trim()) setInputError(false); }} onKeyDown={(e) => { if (e.key === 'Enter') handleGenerate(); }} />
        </div>
        ...
        <div className="w-full xl:w-56 space-y-3 relative z-10">
          ...
          <div className="w-full h-[72px] bg-slate-950 border border-slate-700/50 rounded-2xl relative overflow-hidden shadow-inner flex items-center px-2">
             {masteryLevel > 90 && userTier !== 'Pro' ? (
                <div className="w-full flex flex-col items-center justify-center opacity-50"><span className="text-2xl">🔒</span><span className="text-[8px] uppercase font-bold text-slate-400">{text.proRequired}</span></div>
             ) : (
                 <>
                   ...
                   <div className="absolute inset-0 flex items-center justify-between px-3 z-20">
                      <button onClick={() => changeLevel(-1)} disabled={masteryLevel <= 1} className="w-8 h-8 flex items-center justify-center rounded-full bg-slate-800 text-white hover:bg-slate-700 transition-colors font-bold text-lg">-</button>
                      <div className="flex flex-col items-center relative z-10">
                        <span className="font-bold text-[8px] text-slate-400 uppercase tracking-widest">Lvl</span>
                        <motion.span key={masteryLevel} initial={{ scale: 1.5, color: '#facc15' }} animate={{ scale: 1, color: '#facc15' }} className="font-black text-2xl text-yellow-400 font-display">{masteryLevel}</motion.span>
                      </div>
                      <button onClick={() => changeLevel(1)} disabled={masteryLevel >= 120} className="w-8 h-8 flex items-center justify-center rounded-full bg-slate-800 text-white hover:bg-slate-700 transition-colors font-bold text-lg">+</button>
                   </div>
                 </>
             )}
          </div>
        ...
        <motion.button onClick={() => handleGenerate()} disabled={loading} whileHover={{ scale: 1.02, backgroundColor: '#3b82f6' }} whileTap={{ scale: 0.98 }} className="w-full xl:w-auto px-12 py-5 bg-blue-600 text-white font-black rounded-2xl shadow-xl shadow-blue-500/20 disabled:opacity-50 transition-all text-lg active:scale-95 relative z-10">
        ...
        {loading && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="flex flex-col items-center justify-center py-40 space-y-10 bg-slate-900/20 rounded-[40px] border border-slate-800/40 backdrop-blur-xl">
            ...
            <div className="text-center space-y-3 px-6">
              <p className="text-blue-400 font-black text-2xl tracking-tight font-display">{text.loadingTitle}</p>
          ...
          <div className="md:col-span-2 space-y-10">
            <div className="bg-slate-900/40 p-8 md:p-14 rounded-[40px] border border-slate-800 shadow-2xl backdrop-blur-sm overflow-hidden">
              <div className="flex items-center gap-3 mb-8">
                 <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${viewMode === 'quick' ? 'bg-amber-500/10 text-amber-500 border border-amber-500/20' : 'bg-blue-500/10 text-blue-500 border border-blue-500/20'}`}>{viewMode === 'quick' ? `⚡ ${text.quickMission}` : `📖 ${text.deepDive}`}</span>
                 <span className="px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest bg-slate-800 text-slate-400">{lesson.level}</span>
              </div>
              <h3 className="text-3xl md:text-4xl font-black text-white mb-10 tracking-tight leading-tight font-display">{lesson.title}</h3>
                ...
                {viewMode === 'quick' ? (
                  <motion.div key="quick" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                    <div className="p-8 bg-blue-500/5 rounded-3xl border border-blue-500/10">
                      ...
                      <div className="prose prose-invert max-w-none text-slate-300 text-xl font-medium leading-relaxed whitespace-pre-wrap font-sans">{lesson.summary}</div>
                    </div>
                  ...
                  <motion.div key="deep" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="prose prose-invert max-w-none text-slate-300 leading-relaxed whitespace-pre-wrap text-lg md:text-xl font-medium font-sans">{lesson.content}</motion.div>
                )}
              </AnimatePresence>
            </div>
            ...
            <div id="quiz-area" className="bg-slate-900/40 p-8 md:p-14 rounded-[40px] border border-slate-800 shadow-2xl backdrop-blur-sm relative overflow-hidden">
              ...
              <h3 className="text-2xl font-black mb-10 flex items-center gap-4 text-white relative z-10 font-display">
                <span className="p-3 bg-blue-600/20 rounded-2xl text-blue-400 shadow-inner">📝</span> {text.challengeTitle}
              </h3>
              <div className="space-y-12 relative z-10">
                {activeQuestionIndices.map((qIdx) => {
                  const q = lesson.quiz?.[qIdx];
                  if (!q) return null;
                  const isRetryRound = activeQuestionIndices.length < (lesson.quiz?.length || 0);
                  return (
                    <motion.div key={qIdx} className="space-y-6" initial={isRetryRound ? { opacity: 0, x: -20 } : {}} animate={{ opacity: 1, x: 0 }}>
                      <div className="flex items-center gap-3">
                        ...
                        <p className="font-black text-xl md:text-2xl text-slate-100 tracking-tight font-display">{q.question}</p>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        {q.options?.map((opt) => (
                          <motion.button key={opt} onClick={() => !quizScore && setUserAnswers({ ...userAnswers, [qIdx]: opt })} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} disabled={quizScore !== null} className={`p-6 rounded-2xl border-2 text-left transition-all text-base font-bold shadow-lg ${userAnswers[qIdx] === opt ? 'bg-blue-600/20 border-blue-500 text-blue-400' : 'border-slate-800 bg-slate-950/40 text-slate-400 hover:border-slate-600'} ${quizScore !== null && opt === q.answer ? 'border-green-500 text-green-400 bg-green-500/10' : ''} ${quizScore !== null && userAnswers[qIdx] === opt && opt !== q.answer ? 'border-red-500 text-red-400 bg-red-500/10' : ''}`}>
                            {opt}
                          </motion.button>
                        ))}
                      </div>
              ...
              <div className="mt-16 flex flex-col xl:flex-row items-center justify-between pt-10 border-t border-slate-800 gap-8 relative z-10">
                {quizScore !== null && <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex items-center gap-5 bg-slate-800/60 px-8 py-4 rounded-2xl border border-slate-700 shadow-xl"><p className="text-xl font-black text-white tracking-tight">{text.score}: {quizScore}/{activeQuestionIndices.length}</p></motion.div>}
                {quizScore !== null && quizScore < activeQuestionIndices.length ? (
                  <motion.button onClick={handleRetry} className="w-full xl:w-auto px-16 py-5 bg-orange-500 text-white font-black rounded-2xl shadow-xl">{text.retryBtn} ↺</motion.button>
                ) : quizScore === activeQuestionIndices.length && activeQuestionIndices.length > 0 ? (
                   <motion.button onClick={() => setShowLevelUp(true)} className="w-full xl:w-auto px-16 py-5 bg-yellow-400 text-yellow-950 font-black rounded-2xl shadow-xl">{text.nextLevelBtn} →</motion.button>
                ) : (
                  <motion.button onClick={checkQuiz} disabled={quizScore !== null} className="w-full xl:w-auto px-16 py-5 bg-white text-slate-950 font-black rounded-2xl hover:bg-slate-100 shadow-2xl">{text.finishBtn}</motion.button>
                )}
              </div>
            </div>
          </div>
          <div className="md:col-span-2 xl:col-span-1 space-y-10">
            <motion.div initial={{ x: 30, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="bg-gradient-to-br from-blue-700 to-indigo-700 p-8 md:p-10 rounded-[40px] shadow-2xl relative overflow-hidden border border-white/10">
              <h3 className="text-2xl font-black mb-8 flex items-center gap-4 text-white font-display">{text.powerWords}</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-1 gap-5 relative">
                {lesson.vocabulary?.map((v, idx) => (
                  <motion.div key={idx} whileHover={{ x: 10 }} className="p-6 bg-white/10 rounded-2xl border border-white/10 backdrop-blur-md shadow-lg">
                    <p className="font-black text-white text-xl leading-tight mb-1 font-display">{v.word}</p>
                    <p className="text-blue-100/70 text-sm font-bold italic mb-4">"{v.translation}"</p>
                    <p className="text-white/80 text-xs font-medium leading-relaxed bg-black/20 p-3 rounded-lg font-sans">{v.example}</p>
                  </motion.div>
                ))}
              </div>
            </motion.div>
